...
| 9 | A language task learned from raw prompt bytes | stage A discovers its own lexical unit — which byte opens a bracket, over the full 0–255 alphabet, and where the symbol field starts — unique among 10,496 programs; stage B reaches **0.9986** on string lengths never trained on (one error in 724; corrected from 1.000 per §43) — **pre-audit stream only.** On the re-drawn non-exploitable stream the same method finds **no conforming program**, space exhausted, certificate `complete`: the capability was about *counting*, not balancedness, and a running `min` beside the running `add` restores 1.000 on 859 held-out episodes (§45) | majority constant 0.548, best fitted feature 0.648, random 0.500; gradient descent on the identical spaces conforms **0 of 44 runs** (`research/language-capability/RESULTS.md`) | `scripts/demo.sh --only language` |
