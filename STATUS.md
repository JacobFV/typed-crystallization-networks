...
| 9 | A language task learned from raw prompt bytes | stage A discovers its own lexical unit — which byte opens a bracket, over the full 0–255 alphabet, and where the symbol field starts — unique among 10,496 programs; stage B reaches **0.9986** on string lengths never trained on (one error in 724; the file previously said 1.000, corrected per §43) — **but only on the pre-audit stream; pass `hardening='none'` or the number is not reproducible, FINDINGS §39** | majority constant 0.548, best fitted feature 0.648, random 0.500; gradient descent on the identical spaces conforms **0 of 44 runs** (`research/language-capability/RESULTS.md`) | `scripts/demo.sh --only language` |
