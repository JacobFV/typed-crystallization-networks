...
Requires branch `stranded-block-guard`: `main` has no `close_block`, so arm B
cannot run there. Arm A is `close_block=False`, which is `main`'s behaviour exactly.  Arm B is the
