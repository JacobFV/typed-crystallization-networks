...
1. run the crystallizing arms and record each one's realised `optimizer.step()`
   total -- retraining inside rolled-back trials included, and the residual
   training the plateau gate spends on closed rounds included;
2. run plain argmax once per padded scheduler arm, padded with extra optimizer
   steps on the same objective the crystallizer retrains on, up to that arm's
   total. `argmax0` is the unpadded control and always spends strictly less.
...
from arms import (JOINT_PADDED, JOINT_SCHEDULERS, MIXED_PADDED,  # noqa: E402
                  MIXED_SCHEDULERS)
...
def seed_block(task, seed, budget, noise, arms=None, padded=None, argmax0=True):
    ...
    rows = []
    totals = {}
    schedulers = arms if arms else (MIXED_SCHEDULERS if task == "mixed" else JOINT_SCHEDULERS)
    padded = padded if padded else (MIXED_PADDED if task == "mixed" else JOINT_PADDED)
...
def main():
    ...
    ap.add_argument("--arms", help="comma-separated scheduler arms; default all")
    ap.add_argument("--padded", help="comma-separated scheduler arms to step-match argmax to")
    ap.add_argument("--no-argmax0", action="store_true",
                    help="skip the unpadded control, e.g. when adding one arm to an existing sweep")
    ap.add_argument("--out")
    args = ap.parse_args()

    arms = tuple(args.arms.split(",")) if args.arms else None
    padded = tuple(args.padded.split(",")) if args.padded else None
    out = Path(args.out) if args.out else HERE / "results" / f"{args.task}-{args.budget}.jsonl"
    out.parent.mkdir(parents=True, exist_ok=True)
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = [pool.submit(seed_block, args.task, seed, args.budget, args.noise, arms, padded,
                               not args.no_argmax0)
