...
| scaffold | free nodes | programs | gradient (16 seeds) | median exact error | enumeration | unique | random-draw density |
|---|---|---|---|---|---|---|---|
| `L0_om` | 1 | 4 | **16/16** (100%) | 9.87e-08 | 0.002 s, solved | yes | 0.24 |
| `L1_om_s` | 2 | 12 | **16/16** (100%) | 9.87e-08 | 0.001 s, solved | yes | 0.087 |
| `L1b_+t2` | 3 | 36 | **12/16** (75%) | 9.87e-08 | 0.002 s, solved | no | 0.068 |
| `L1c_+t1` | 4 | 72 | **11/16** (69%) | 9.87e-08 | 0.016 s, solved | no | 0.033 |
| `L1d_+csq` | 5 | 144 | **8/16** (50%) | 0.0194 | 0.007 s, solved | no | 0.013 |
| `L2_trig` | 6 | 288 | **0/16** (0%) | 0.243 | 0.014 s, solved | no | 0.0064 |
| `L2b_+c` | 7 | 864 | **0/16** (0%) | 0.32 | 0.041 s, solved | no | 0.0019 |
| `L3_trig_alg` | 9 | 12,960 | **0/16** (0%) | 0.388 | 1.341 s, solved | no | 0.00018 |
| `L4_all` | 12 | 829,440 | **0/16** (0%) | 0.65 | 76.583 s, solved | no | 2e-05 |
...
| scaffold | relaxed loss at the one-hot reference | median relaxed loss the optimiser reaches | best of 8 |
|---|---|---|---|
| `L0_om` | 3.04e-16 | 1.3e-06 | 4.38e-07 |
| `L1_om_s` | 3.04e-16 | 2.7e-06 | 5.56e-07 |
| `L1b_+t2` | 3.04e-16 | 0.000121 | 1.96e-05 |
| `L1c_+t1` | 3.04e-16 | 1.75e-07 | 4.66e-08 |
| `L1d_+csq` | 3.04e-16 | 9.62e-05 | 3.9e-06 |
| `L2_trig` | 3.04e-16 | 0.00887 | 0.00212 |
| `L2b_+c` | 3.04e-16 | 0.00367 | 0.000541 |
| `L3_trig_alg` | 3.04e-16 | 0.00768 | 0.00423 |
| `L4_all` | 3.04e-16 | 0.00438 | 0.00386 |
...
| arm | supervision | programs | gradient (12 seeds) | median held-out max error |
|---|---|---|---|---|
| `dense_freq+future_pinned_recurrence` | future + frequency | 829,440 | 0/12 | 112.6 |
| `dense_freq+future` | future + frequency | 13,589,544,960 | 0/12 | 5.237 |
| `output_only_same_space_pinned_recurrence` | future | 829,440 | 0/12 | 123.3 |
| `output_only_same_space` | future | 13,589,544,960 | 0/12 | 1104 |
| `output_only_no_branch_pinned_recurrence` | future | 2,880 | 0/12 | 3.71 |
| `output_only_no_branch` | future | 47,185,920 | 0/12 | 5.584 |
...
| arm | programs | supervision | exact on train (8 seeds) | mean held-out accuracy | enumeration | random density |
|---|---|---|---|---|---|---|
| `R0_fold_only` | 65,536 | dense | **8/8** | 0.9570 | 33.07 s, solved=True | 0.0015 |
| `R0_fold_only` | 65,536 | output_only | **8/8** | 0.9512 | 33.07 s, solved=True | 0.0015 |
| `R1_pool4` | 65,536 | dense | **8/8** | 0.9434 | 46.03 s, solved=True | 9e-05 |
| `R1_pool4` | 65,536 | output_only | **1/8** | 0.8730 | 46.03 s, solved=True | 9e-05 |
| `R2_pool16` | 4,294,967,296 | dense | **0/8** | 0.9180 | space too large | 1e-05 |
| `R2_pool16` | 4,294,967,296 | output_only | **0/8** | 0.8770 | space too large | 1e-05 |
| `R3_free_wiring` | 281,474,976,710,656 | dense | **0/8** | 0.9160 | space too large | 1e-05 |
| `R3_free_wiring` | 281,474,976,710,656 | output_only | **0/8** | 0.8750 | space too large | 1e-05 |
...
| resolution | observation width | byte addresses | programs | gradient (12 seeds) | enumeration | held-out error of the enumerated program | random density |
|---|---|---|---|---|---|---|---|
| 1x1 | 6 | 3 | 36 | **5/12** (42%) | 0.01 s, solved | 0 | 0.13 |
| 2x2 | 15 | 12 | 576 | **1/12** (8%) | 0.27 s, solved | 0 | 0.0071 |
| 3x3 | 30 | 27 | 2,916 | **1/12** (8%) | 0.25 s, solved | 0 | 0.0042 |
| 4x4 | 51 | 48 | 9,216 | **2/12** (17%) | 1.96 s, solved | 0 | 0.002 |
| 6x6 | 111 | 108 | 46,656 | **0/12** (0%) | 10.41 s, solved | 0 | 0.0013 |
| 8x8 | 195 | 192 | 147,456 | **0/12** (0%) | 71.28 s, solved | 0 | 0.0011 |
...
| resolution | programs (address pinned, colour searched over all 256 byte values) | gradient (12 seeds) | enumeration |
|---|---|---|---|
| R2 | 65,536 | **12/12** | 7.87 s, solved=True, unique=yes |
| R4 | 65,536 | **12/12** | 14.07 s, solved=True, unique=yes |
...
`eq` route (the only differentiable predicate on bytes): 49,152 programs. Enumeration **exhausted the space in 51.7 s and found no conforming program** (solved=False, exhausted=True). Gradient synthesis: 0/8.

Gradient reach in the `pack -> decode -> lt` program: `bytes` = None, `px` = None, `wrap` = None, `packed` = 0.0, `value` = 0.0, `hit` = 0.0; trainable threshold gradient = {'thr': 0.0}.

| arm | exact on train | median held-out error |
|---|---|---|
| init128.0_lr0.05 | 0/8 | 1 |
| init128.0_lr2.0 | 0/8 | 1 |
| init70.0_lr0.05 | 0/8 | 1 |
| pinned_address_init128.0_lr0.05 | 0/8 | 1 |
| pinned_address_init128.0_lr2.0 | 0/8 | 1 |

Discrete backend over address x comparison x 256 thresholds (98,304 programs): solved=True in 202.7 s, unique=no, held-out accuracy 1.0000; random search density 0.0013.
...
| head | resolution | programs | P(all background) | supervision | exact on train (12 seeds) | **generalises to 48 held-out episodes** |
|---|---|---|---|---|---|---|
| `mask` | R2 | 1,048,576 | 0.458 | dense | 12/12 | **12/12** |
| `mask` | R2 | 1,048,576 | 0.458 | output_only | 12/12 | **12/12** |
| `mask` | R3 | 35,184,372,088,832 | 0.208 | dense | 12/12 | **12/12** |
| `mask` | R3 | 35,184,372,088,832 | 0.208 | output_only | 12/12 | **12/12** |
| `allbg` | R2 | 8,388,608 | 0.458 | dense | 12/12 | **12/12** |
| `allbg` | R2 | 8,388,608 | 0.458 | output_only | 3/12 | **0/12** |
| `allbg` | R3 | 9,007,199,254,740,992 | 0.208 | dense | 12/12 | **12/12** |
| `allbg` | R3 | 9,007,199,254,740,992 | 0.208 | output_only | 9/12 | **1/12** |
...
| head | resolution | programs | P(all background) | supervision | exact on train (12 seeds) | **generalises to 48 held-out episodes** |
|---|---|---|---|---|---|---|
| `mask` | R2 | 1,761,205,026,816 | 0.458 | dense | 0/12 | **0/12** |
| `mask` | R2 | 1,761,205,026,816 | 0.458 | output_only | 0/12 | **0/12** |
| `mask` | R3 | 7,804,725,584,345,565,904,628,551,916,716,032 | 0.208 | dense | 0/12 | **0/12** |
| `mask` | R3 | 7,804,725,584,345,565,904,628,551,916,716,032 | 0.208 | output_only | 0/12 | **0/12** |
| `allbg` | R2 | 14,089,640,214,528 | 0.458 | dense | 0/12 | **0/12** |
| `allbg` | R2 | 14,089,640,214,528 | 0.458 | output_only | 0/12 | **0/12** |
| `allbg` | R3 | 1,998,009,749,592,464,871,584,909,290,679,304,192 | 0.208 | dense | 0/12 | **0/12** |
| `allbg` | R3 | 1,998,009,749,592,464,871,584,909,290,679,304,192 | 0.208 | output_only | 0/12 | **0/12** |
