"""Joint prediction+policy fixture (`examples/joint.py`) under each commitment schedule.
...
tolerance 0.05, retrain_steps 2.  Evaluation is the deterministic mean return
over 16 held-out test episodes plus, when the graph is fully frozen, the exact
frozen-program return over 16 further test episodes.

Environment cost: one objective evaluation is two validation rollouts, so
`loss_evaluations * 2` is the crystallizer's own environment budget on top of the
N training episodes.  That is FINDINGS F-budget's undisclosed cost, counted here.

This mirrors `research/loss-gated-eligibility/run_joint.py` so the two tracks'
...
def run(arm, seed, episodes=EPISODES, extra_steps=0):
    ...
    if arm.startswith("argmax"):
        ...
        objective = Objective(validation, lambda: validation(False))
        try:
            scheduler.run(objective, rounds=rounds_for(arm), retrain_steps=RETRAIN_STEPS)
        except Exception as exc:  # noqa: BLE001
    ...
    # One objective evaluation is two validation rollouts.
    record["crystallization_episodes"] = 2 * evals["loss"]
...
def main():
    ...
    print(json.dumps(record) if not args.out else Path(args.out).write_text(json.dumps(record)))
