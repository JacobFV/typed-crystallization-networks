...
| R | pixels | foreground fraction | `depth==0` iff no object | held-out lookup acc (foreground) | held-out lookup acc (`object_ids`) | majority baseline | colliding RGB (exact depth) |
|---|---|---|---|---|---|---|---|
| 4 | 16 | 0.51 | yes | 1 | 0.5625 | 0.5625 | 0.254 |
| 8 | 64 | 0.514 | yes | 1 | 0.5677 | 0.5677 | 0.532 |
| 12 | 144 | 0.509 | yes | 1 | 0.5637 | 0.5637 | 0.605 |
| 16 | 256 | 0.513 | yes | 1 | 0.5703 | 0.5703 | 0.659 |
| 24 | 576 | 0.512 | yes | 1 | 0.5677 | 0.5677 | 0.782 |
| 32 | 1024 | 0.512 | yes | 1 | 0.5696 | 0.5696 | 0.839 |
...
| R | obs width | positions | supervised records | space | programs evaluated | exhausted | unique | enum s | stop-at-first | held-out err (records) | held-out err (whole probe) | count err | caller nodes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 4 | 48 | 16 | 128 | 32,000 | 32,000 | yes | no | 61.8 | 21 / 0.04s | 0 | 1 | 3 | 3 |
| 8 | 192 | 64 | 384 | 32,000 | 32,000 | yes | no | 1087.9 | 21 / 1.35s | 0 | 1 | 8 | 3 |
...
| R | gradient successes (init_noise 0.5) | median s | random-draw density | worst held-out error over seeds | exhaustive enumeration s |
|---|---|---|---|---|---|
| 4 | 4/4 | 22.0 | 0.0785 | 0 | 61.8 |
| 8 | 4/4 | 178.5 | 0.0825 | 0 | 1087.9 |
...
searched once at R=8: **1952** programs conform on training records, **1640** also on validation, out of **32,000**, in 887.0s

| R | obs width | positions | caller nodes | space | max error over dense probe | count error | apply s/image | fresh `enumerate_fit` projection (s) |
|---|---|---|---|---|---|---|---|---|
| 8 | 192 | 64 | 3 | 32,000 | 0 | 0 | 0.08 | 384.6 |
| 16 | 768 | 256 | 3 | 32,000 | 0 | 0 | 1.17 | 569.3 |
| 24 | 1,728 | 576 | 3 | 32,000 | 0 | 0 | 5.22 | 2097 |
| 32 | 3,072 | 1,024 | 3 | 32,000 | 0 | 0 | 12.72 | 1900 |
| 48 | 6,912 | 2,304 | 3 | 32,000 | 0 | 0 | 64.03 | 2602 |
...
| space | programs evaluated | exhausted | unique | enum s | held-out max error | gradient successes | gradient median s |
|---|---|---|---|---|---|---|---|
| 288,000 | 288,000 | yes | no | 109.1 | 0 | 1/4 | 3.7 |
...
| R | supervised records | conforming | also exact on held-out | unique | fraction that fail held-out |
|---|---|---|---|---|---|
| 4 | 16 | 2,752 | 2,608 | no | 0.0523 |
| 4 | 64 | 2,608 | 2,472 | no | 0.0521 |
| 4 | 128 | 2,464 | 2,336 | no | 0.0519 |
| 8 | 16 | 2,752 | 2,472 | no | 0.102 |
| 8 | 64 | 2,752 | 2,472 | no | 0.102 |
| 8 | 256 | 2,608 | 2,336 | no | 0.104 |
| 8 | 384 | 2,608 | 2,336 | no | 0.104 |
...
| R | space | conforming on train | + validation | selection rule | test accuracy (whole probe) | test max error | wrong slots |
|---|---|---|---|---|---|---|---|
| 4 | 32,000 | 2,464 | 2,336 | lexicographic | 0.992188 | 1.0 | 3/384 |
| 4 | 32,000 | 2,464 | 2,336 | validation_filtered | 0.992188 | 1.0 | 3/384 |
| 4 | 32,000 | 2,464 | 2,336 | gradient | 1.000000 | 0.0 | 0/384 |
...
| arm | space | programs evaluated | exhausted | unique | enum s | held-out max error | held-out accuracy | gradient |
|---|---|---|---|---|---|---|---|---|
| staged (frozen module called twice) | 48 | 48 | yes | yes | 4.608 | 0 | 1 | 1/4 |
| flat (nothing frozen) | 49,152,000,000 | 40,000 | no | -- | 195.0 | -- | -- | 0/3 |
...
| target | classes | distinct RGB (train) | colliding RGB | train accuracy | held-out accuracy | majority baseline | advantage |
|---|---|---|---|---|---|---|---|
| foreground | 2 | 87 | 0 | 1 | 1 | 0.4492 | 0.551 |
| object_ids | 7 | 87 | 0 | 1 | 0.5508 | 0.5508 | 0 |
| is_object_0 | 2 | 87 | 0 | 1 | 0.8574 | 0.8574 | 0 |
| raster_rank | 6 | 87 | 0 | 1 | 0.5508 | 0.5508 | 0 |
| depth_bin | 3 | 87 | 4 | 0.9863 | 0.5508 | 0.5508 | 0 |
...
mean distinct colours per object **3.31**, max **18**, single-coloured objects **34.6%** (26 object instances)

### 6.2 Exhaustive search in every family the algebra allows

| family / target | space | programs evaluated | exhausted | solved | unique | s | held-out accuracy |
|---|---|---|---|---|---|---|---|
| colour-constant / foreground | 32,000 | 32,000 | yes | yes | no | 425.9 | 1 |
| colour-constant / is_object_0 | 32,000 | 32,000 | yes | no | no | 2137.6 | -- |
| relational / foreground | 16,384 | 16,384 | yes | no | no | 84.5 | -- |
| relational / is_object_0 | 16,384 | 16,384 | yes | no | no | 901.1 | -- |
| multi-class relational R6 | 46,656 | 46,656 | yes | no | no | 19.4 | -- |
...
The underlying obstruction is measurable directly: mean distinct colours per object **3.31**, max **18**, single-coloured objects **34.6%** (26 object instances)  An object
...
| information source | distinct keys | train accuracy | held-out accuracy | majority baseline | advantage |
|---|---|---|---|---|---|
| colour | 87 | 0.9883 | 0.8105 | 0.8105 | 0 |
| position | 64 | 0.7422 | 0.8105 | 0.8105 | 0 |
| colour_and_position | 329 | 1 | 0.8105 | 0.8105 | 0 |

| family | space | programs evaluated | exhausted | solved | s |
|---|---|---|---|---|---|
| colour constants (rung-3 family) | 32,000 | 32,000 | yes | no | 30.6 |
| position threshold x colour | 15,360,000 | 100,000 | no | no | 109.1 |
| position threshold, colour pinned | 480 | 480 | yes | no | 0.4 |

No row is solved, so `unique` is undefined for all of them; an exhausted, unsolved sweep is a completeness certificate that the family contains no exact program.

best-accuracy program in the 480-program position family: train **0.8203**, held-out **0.8086**, against a majority baseline of **0.8105**
...
| method | space searched | programs evaluated | exhausted | unique | solved | s | projected exhaustive s |
|---|---|---|---|---|---|---|---|
| exhaustive enumeration (`enumerate_fit`) | 4,294,967,296 | 100,000 | no | -- | yes | 215.0 | 9.23e+06 |
| observed-alphabet scoping, then enumeration | 549,552,384 | 100,000 | no | -- | yes | 352.6 | 1.94e+06 |
| coordinate descent, random restarts | 4,294,967,296 | 1,601 | no | -- | 1/6 | 62.2 | -- |
| gradient descent (init_noise 0.5) | 4,294,967,296 | -- | no | -- | 6/6 | 600.8 | -- |
...
| R | obs width | records | space | node evaluations | prefix-reusing s | conforming | `enumerate_fit` s | speedup | identical conforming set |
|---|---|---|---|---|---|---|---|---|---|
| 8 | 192 | 384 | 32,000 | 36,770 | 60.0 | 2,608 | 1067.9 | 18x | yes |
| 16 | 768 | 384 | 32,000 | 36,050 | 71.6 | 1,888 | -- | -- | -- |
| 32 | 3,072 | 384 | 32,000 | 35,826 | 47.8 | 1,664 | -- | -- | -- |
| 64 | 12,288 | 384 | 32,000 | 36,770 | 74.2 | 2,608 | -- | -- | -- |
