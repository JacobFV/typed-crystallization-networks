...
| **structural generalization** | 3.38–4.00 on gate families never trained on *(8-seed arm means; per-seed floor 2.75 — §62 audit)*; interpreter candidate selected 8/8 unprompted | best constant 2.13–2.56; the recorded scaffold measures 1.95–2.03 and cannot exceed 2.00 |
...
| **recursive abstraction** | module on the output path in 27/27 successes *(chance rate 0.816 tight / 0.6886 wide)* | flat arm and distractor arm both 0; the 230,400-program flat space is exhausted with no solution; 144 of 144 solutions use the module |
| **positional reuse** | one frozen module at 1,024 positions of a 3,072-value observation with 3 caller nodes (2,304 over 6,912 values under `--full`) | 17 structural symbols shared against 3N+13 per-position and 8N+1 inlined, at 0.93× execution cost *(N=4; 0.91× at the demo's N=32 — §62 audit)* |
