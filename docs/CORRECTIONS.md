...
| 37 | Admitting a schema on the training episodes it conforms to | **Not enough.** §65's inherited schema conforms on 2.6×10¹¹ programs at `gap 1` and contains **zero** that conform on all 48 episodes — its STEP pool lacks the needed two-row step. A schema admitted on training conformance can be refuted by the held-out set, which is where CEGIS gets its counterexample | The flagship's exact 48-episode count at `gap 1`, exhausted with certificate `complete` | §64, §65 |
| 38 | "The inherited library made the integrated task cheaper", in any form | **Not shown, and the pre-registered form is false.** §65: C1 fails by 10,925×; post-hoc, the library beats the schema-only arm by 292× but is *not* shown to beat the flat substrate (its bound lies below), and a hand-feature control is 31× cheaper. What generalizes is one estimator fitted on 20 rows, and a one-line human rule reproduces it | The flagship's pre-registered arms plus its post-hoc knockouts and hand-rule controls | §65 |
