...
class JointTrainer:
    ...
    def episode(self,index,train=True,split='train',loss_only=False,regularized=True):
        ...
        # Regularizers are excluded from the task objective so the connectivity
        # guard probes learning signal rather than graph reachability; description
        # cost is a regularizer and belongs on the same side of that line.
        if regularized:
            loss=loss+c.mdl_weight*self.model.complexity()+c.crystal_weight*self.model.entropy()
